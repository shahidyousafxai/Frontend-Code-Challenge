...
import { checkoutItem, deleteCheckoutItem, deleteItem } from '../../services/api';

const ProductCard = ({ product, getItemsListing, getCheckoutProducts }) => {
  ...
  // Delete Product
  const deleteProduct = async (id, checked) => {
    try {
      if(checked) {
        const response = await deleteCheckoutItem(id);
        console.log(response);
        if (response?.status) {
          getCheckoutProducts();
        }
    ...
    <div className="group relative border p-2 rounded-lg bg-gray-100 shadow-md">
      ...
      <div className="mt-4 text-base font-medium text-gray-900">
        <div className={`flex items-center ${product?.checkout ? "justify-end" : "justify-between"}`}>
          ...
          onClick={() => deleteProduct(product?.id, product?.checkout)}
          >
            <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={1.5} stroke="currentColor" className="w-5 h-5">
